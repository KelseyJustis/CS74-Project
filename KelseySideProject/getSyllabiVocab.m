...
folderOfSyllabiTxtFiles = dir('SyllabiTxtFiles/*.txt');
numOfFeatures = 5;
numOfCourses =  length(folderOfSyllabiTxtFiles);
syllabiVocab = cell(numOfCourses,numOfFeatures);
...
for syllabusFileNum = 1:numOfCourses
    ...
    CurrSyllabusFileID = fopen(['SyllabiTxtFiles/'  CurrSyllabusFileName], 'r');
  ...
  syllabiVocab{syllabusFileNum,1} = courseDept; 
  syllabiVocab{syllabusFileNum,2} = courseName;
  syllabiVocab{syllabusFileNum,3} = courseNum;
  syllabiVocab{syllabusFileNum,4} = syllabusWordCount; % Number of words in syllabus
  syllabiVocab{syllabusFileNum,5} = syllabusWordDistribution; % Words present and frequency of usage
  
  save('syllabiVocab','syllabiVocab');
  fclose('all');
end
fprintf('Dictionary generation complete.')